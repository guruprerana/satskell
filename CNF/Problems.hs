...
frm =
  BigAnd
    [1 .. 2]
    [BigOr [Lit x True, Lit y True], BigOr [Lit x False, Lit y False]]
...
kcolor k (vs, es) =
  ...
  [ BigOr [Lit (hascol v c) False, Lit (hascol u c) False]
  | (v, u) <- es
  , c <- [1 .. k]
  ]
  where
...
prismGraph =
  ( [1 .. 6]
  , [(1, 2), (1, 3), (1, 4), (2, 3), (2, 6), (3, 5), (4, 5), (4, 6), (5, 6)])
...
php m n =
  ...
  [ BigOr [Lit (pigeon p1 h) False, Lit (pigeon p2 h) False]
  | p1 <- [1 .. m]
  , p2 <- [1 .. p1 - 1]
  , h <- [1 .. n]
  ]
